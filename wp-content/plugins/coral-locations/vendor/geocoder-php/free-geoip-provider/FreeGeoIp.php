...
final class FreeGeoIp extends AbstractHttpProvider implements Provider
{
    ...
    public function geocodeQuery(GeocodeQuery $query)
    {
        ...
        if (!empty($data['region_name'])) {
            $builder->addAdminLevel(1, $data['region_name'], $data['region_code'] ? $data['region_code'] : null);
        }

        if ($data['latitude'] !== 0 || $data['longitude'] !== 0) {
            $builder->setCoordinates($data['latitude'] ? $data['latitude'] : null, $data['longitude'] ? $data['longitude'] : null);
        }
